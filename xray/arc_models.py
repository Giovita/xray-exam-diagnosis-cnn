...
def load_and_process(data_folder,archivo, cols):
    ...
    xray_df['Enfermo'] = np.where(xray_df['Count_diseases'] == 0, 'False', 'True') # se utiliza en string por keras
    ...
    xray_df['Fixed_Labels'] = xray_df['Combined'].apply(lambda x: '|'.join([cols[i] for i, val in enumerate(x) if val == 1]))
...
def build_model(train_gen,
    ...
    vgg_model = model_name(include_top=False,
                      weights='imagenet',
