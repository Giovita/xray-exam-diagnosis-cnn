...
def process_df(dataframe: pd.DataFrame, cols, save_df_to_csv=False):
    ...
    """
    # N. images per classification
    dataframe["count_diseases"] = dataframe["Finding Labels"].map(lambda x: len(x.split("|")))
    dataframe["Count_diseases"] = np.where(dataframe["Finding Labels"] == "No Finding", 0, dataframe["count_diseases"])

    dataframe.drop(columns="count_diseases", inplace=True)

    # Label encoder
    dataframe["Enfermo"] = np.where(dataframe["Count_diseases"] == 0, "False", "True")  # se utiliza en string por keras
    ...
    for c_label in all_labels:
        if len(c_label) > 1:
            dataframe[c_label] = dataframe["Finding Labels"].map(lambda finding: 1.0 if c_label in finding else 0)
...
def filter_dataset_from_list(dataset: pd.DataFrame, filter_list: list, filter_by_col: str):
    """
    ...
    bool_filter = dataset[filter_by_col].isin(values_to_filter[values_to_filter.column[0]])
...
def split_df(
    ...
    * By default will give a train_val split. If a tuple is provided as train_val_test,
    ...
    if not column_to_filter_by:
        ...
        patients = pd.DataFrame(dataset[column_to_filter_by].unique(), columns=[column_to_filter_by])

    # Filtered patients
    reduced_patients = patients.sample(
        frac=total_filter,
    )

    # Reduce full DS
    reduced_ds = dataset[dataset[column_to_filter_by].isin(reduced_patients[column_to_filter_by])]
    ...
    patients_shuffle = reduced_patients.sample(frac=1)  # Randomize patients list
    ...
    ds_train = reduced_ds[reduced_ds[column_to_filter_by].isin(patients_train[column_to_filter_by])]
    ds_val = reduced_ds[reduced_ds[column_to_filter_by].isin(patients_val[column_to_filter_by])]
    ds_test = reduced_ds[reduced_ds[column_to_filter_by].isin(patients_test[column_to_filter_by])]
