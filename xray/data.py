...
def process_df(dataframe: pd.DataFrame, cols, save_df_to_csv=False):
    ...
    dataframe["Count_diseases"] = np.where(
        dataframe["Finding Labels"] == "No Finding", 0, dataframe["count_diseases"]
    )
    ...
    dataframe["Enfermo"] = np.where(
        dataframe["Count_diseases"] == 0, "False", "True"
    )  # se utiliza en string por keras

    # Multiple encoding de las clases
    all_labels = np.unique(
        list(chain(*dataframe["Finding Labels"].map(lambda x: x.split("|")).tolist()))
    )
    ...
    index_mayor100 = list(
        dataframe[dataframe["Patient Age"] > 100].index
...
def filter_dataset_from_list(
    dataset: pd.DataFrame, filter_list: list, filter_by_col: str
):
    """
    ...
    * By default will give a train_val split. If a tuple is provided as train_val_test,
    ...
    if not column_to_filter_by:
        ...
        patients = pd.DataFrame(
            dataset[column_to_filter_by].unique(), columns=[column_to_filter_by]
        )
    ...
    # Reduce full DS
    reduced_ds = dataset[
        dataset[column_to_filter_by].isin(reduced_patients[column_to_filter_by])
    ]
    ...
    ds_train = reduced_ds[
        reduced_ds[column_to_filter_by].isin(patients_train[column_to_filter_by])
    ]
    ds_val = reduced_ds[
        reduced_ds[column_to_filter_by].isin(patients_val[column_to_filter_by])
    ]
    ds_test = reduced_ds[
        reduced_ds[column_to_filter_by].isin(patients_test[column_to_filter_by])
    ]
