...
def filter_datraset_from_list(dataset: pd.DataFrame, filter_list: list, filter_by_col: str):
    """
...
def split_train_val(dataset: pd.DataFrame, column_to_filter, split: float = 0.85):
    """
    ...
    patients = pd.DataFrame(dataset[column_to_filter].unique(), columns=[column_to_filter])

    train_guide = patients.sample(frac=0.8, random_state=42)
